...
if __name__ == '__main__':
    ...
    print('> Show bacteria configuration...')
    print('\t The pushing speed: %s [um/sec]' %(VF.Vp))
    print('\t The wrapping speed: %s [um/sec]' %(VF.Vw))
    print('\t The pushing time0: %s [sec]' %(VF.tau_ccw0))
    print('\t The wrapping time0: %s [sec]' %(VF.tau_cw0))
    print('\t The syncronization probability: %s' %(VF.sync_prob0))
    print('\t Total simulation cells: %d' %(nCells))
    print('\t The simulation time steps: %d' %(VF.nSteps))
    
    # simulate a bacterial swimming process
    ###########################################################################
    # print('> Start simulating a bacterial swimming process with low syncronization under homogeneous environment...')
    # tic=time.time()
    # pool = multiprocessing.Pool()
    # cells1=pool.map(functools.partial(VF.path_generator,is_che=False,is_high_sync=False),range(nCells))
    # pool.close()
    # pool.join()
    # print("--- (Simulation time: %s seconds) ---" % (time.time()-tic))
    
    # tic=time.time()
    # VF.paths_plot(cells1,is_all=True)
    # VF.drift_dir_calc(cells1)
    # VF.T_calc(cells1)
    # print("--- (Analysis time: %s seconds) ---" % (time.time()-tic))
    
    ###########################################################################
    # print('> Start simulating a bacterial swimming process with high synchronization under homogeneous environment...')
    # tic=time.time()
    # pool = multiprocessing.Pool()
    # cells2=pool.map(functools.partial(VF.path_generator,is_che=False,is_high_sync=True),range(nCells))
    # pool.close()
    # pool.join()
    # print("--- (Simulation time: %s seconds) ---" % (time.time()-tic))
    
    # tic=time.time()
    # VF.paths_plot(cells2,is_all=True)
    # VF.drift_dir_calc(cells2)
    # VF.T_calc(cells2)
    # print("--- (Analysis time: %s seconds) ---" % (time.time()-tic))
    
    ###########################################################################
    # print('> Start simulating a bacterial swimming process with low synchronization under gradient environment...')
    # tic=time.time()
    # pool = multiprocessing.Pool()
    # cells3=pool.map(functools.partial(VF.path_generator,is_che=True,is_high_sync=False),range(nCells))
    # pool.close()
    # pool.join()
    # print("--- (Simulation time: %s seconds) ---" % (time.time()-tic))
    
    # tic=time.time()
    # VF.paths_plot(cells3,is_all=True)
    # VF.drift_dir_calc(cells3)
    # VF.arrival_calc(cells3)
    # VF.T_calc(cells3)
    # print("--- (Analysis time: %s seconds) ---" % (time.time()-tic))
    ...
    VF.drift_dir_calc(cells4)
    VF.arrival_calc(cells4)
    VF.T_calc(cells4)
    print("--- (Analysis time: %s seconds) ---" % (time.time()-tic))
